...
Additionally, `nadamu-lifecycle` should remain decoupled from `nadamu-auth`, functioning seamlessly in standalone mode or with alternative auth setups.

## Decision
1. **Optional Soft Dependency**:
   `nadamu-lifecycle` declares an optional dependency on `nadamu-auth` (`@Dependency(id = "nadamu-auth", optional = true)`).
...
2. **Explicit Transfer Invariant**:
   `nadamu-lifecycle` never executes a server transfer (`connectWithIndication`) for any player whose state is `PENDING_LOGIN`.
3. **Defense-in-Depth via Velocity PostOrder**:
   - `nadamu-lifecycle` hooks into `PlayerChooseInitialServerEvent` and `ServerPreConnectEvent` at `PostOrder.FIRST` or `PostOrder.EARLY`.
...
- Completely decoupled codebase: `nadamu-lifecycle` can be deployed independently.
